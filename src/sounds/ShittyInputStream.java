...
{
	private int length = 0; // the fucking byte length of the file
	...
	public ShittyInputStream(InputStream bound, int length)
	{
		...
		if(length > 0)
		{
			super.mark(length); // mark the location of the stream at the beginning and set the invalidation size to be the size of the file
		}
	}
	...
	// say "Fuck no" when something (i.e. JLayer) tries to close the stream programmatically since
	...
	public void close()
	{
		...
		if(length > 0)
		{
			try
			{
				...
				super.reset(); // will throw an exception here when closeManually() is used if the player is running, but who the hell cares?
			}
			catch(Exception e)
			{
